...
""" srun -n 1 --gres=gpu:1 --cpus-per-task=15 --partition=ml-all-gpu --output=steam-3.txt python -u renttherunway-3.py & """
...
num_tours = 80
batch_size = 5
super_node_size = 100000
cuda = False

train = {"Hyperedge": torch.load("data/"+dataset+"/train.pt")[k]["Hyperedge"]}
test =  {"Hyperedge": torch.load("data/"+dataset+"/test.pt")[k]["Hyperedge"]}

graph = torch.load("data/"+dataset+"/train_data.pt")["graph"]
test_graph = torch.load("data/"+dataset+"/test_data.pt")["graph"]
dataset = torch.load("data/"+dataset+"/train_data.pt")["dataset"]
...
train_subsample_ids = list( range( len( train["Hyperedge"]["x"]  ) ) )
test_subsample_ids = list( range( len( test["Hyperedge"]["x"]  ) ) )
random.shuffle(train_subsample_ids)
random.shuffle(test_subsample_ids)
train_subsample_ids = train_subsample_ids[:10000]
test_subsample_ids = test_subsample_ids[:10000]
...
for i in range(0, len(dataset), batch_size):
    ...
    train_x=best_model.embedding([ get_subgraph(graph, train["Hyperedge"]["x"][i].squeeze()) for i in train_subsample_ids ]).detach().cpu(), train_y=train["Hyperedge"]["y"][train_subsample_ids], \
    test_x=best_model.embedding([ get_subgraph(test_graph, test["Hyperedge"]["x"][i].squeeze()) for i in test_subsample_ids ]).detach().cpu(), test_y=test["Hyperedge"]["y"][test_subsample_ids] ))
