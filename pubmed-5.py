...
for i in range(0, len(dataset), batch_size):
    ...
    for i in train_subsample_ids ]).detach().cpu(), train_y=train["DAG"]["y"][train_subsample_ids], \
      test_x=model.embedding([ get_subgraph(graph, test["DAG"]["x"][i].squeeze()) for i in test_subsample_ids ]).detach().cpu(), test_y=test["DAG"]["y"][test_subsample_ids] ))
