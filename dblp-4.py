...
""" srun -n 1 --gres=gpu:1 --cpus-per-task=15 --partition=ml-all-gpu --output=dblp-4.txt python -u dblp-4.py & """
...
for i in range(0, len(dataset), batch_size):
    ...
    if loss.item() <= best_loss - tol:
        ...
        cn_wait += 1
    print("loss:\t", loss.item(), "time:\t", time.time()-stime, "step:\t", step, "hyperedge:\t", evaluate.logit(train_x=model.embedding([ get_subgraph(graph, train["Hyperedge"]["x"][i].squeeze()) \
    for i in train_subsample_ids ]).detach().cpu(), train_y=train["Hyperedge"]["y"][train_subsample_ids], \
      test_x=model.embedding([ get_subgraph(graph, test["Hyperedge"]["x"][i].squeeze()) for i in test_subsample_ids ]).detach().cpu(), test_y=test["Hyperedge"]["y"][test_subsample_ids] ))
